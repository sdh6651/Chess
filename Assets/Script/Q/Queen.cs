...
public class Queen : MonoBehaviour {
    ...
    public GameObject movep4;
    public GameObject movep5;
    public GameObject movep6;
    public GameObject movep7;
    ...
    void Update()
    {
        if (pOnOff == true)
        {
            ...
            mz = whoObj.transform.position.z + 0.5f;
            Instantiate(movep4, new Vector3(mx, my, mz), Quaternion.Euler(0, 0, 0));
            my = whoObj.transform.position.y + 0.15f;
            mx = whoObj.transform.position.x;
            mz = whoObj.transform.position.z - 0.5f;
            Instantiate(movep5, new Vector3(mx, my, mz), Quaternion.Euler(0, 0, 0));
            my = whoObj.transform.position.y + 0.15f;
            mx = whoObj.transform.position.x - 0.5f;
            mz = whoObj.transform.position.z;
            Instantiate(movep6, new Vector3(mx, my, mz), Quaternion.Euler(0, 0, 0));
            my = whoObj.transform.position.y + 0.15f;
            mx = whoObj.transform.position.x + 0.5f;
            mz = whoObj.transform.position.z;
            Instantiate(movep7, new Vector3(mx, my, mz), Quaternion.Euler(0, 0, 0));
            pOnOff = false;
        }
        if (moveObj == true)
        {
